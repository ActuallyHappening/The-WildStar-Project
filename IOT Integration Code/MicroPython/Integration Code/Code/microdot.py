...
class URLPattern():
    ...
    def __str__(self):
        return f"URLPattern(args={self.args}, pattern={self.pattern})"
...
class Microdot():
    ...
    def routes(self, url_pattern, methods=None):
        def decorated(f):
            if url_pattern[-1:] == '/':  # Adds second route (without '/')
                self.url_map.append(
                    (methods or ['GET'], URLPattern(url_pattern[:-1]), f))
            self.url_map.append(
                (methods or ['GET'], URLPattern(url_pattern), f))
            return f
        return decorated
